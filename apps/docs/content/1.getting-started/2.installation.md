...
evlog provides a Fastify plugin at `evlog/fastify`. It auto-creates a request-scoped logger accessible via `request.log` or `useLogger()` and emits a wide event when the response finishes.
...
::callout{icon="i-lucide-info" color="info"}
**Note:** `request.log` is the evlog wide-event logger (shadows Fastify's built-in pino logger on the request). Fastify's pino logger remains accessible via `fastify.log` for server-level structured logging.
::

::callout{icon="i-lucide-arrow-right" color="neutral"}
See the [Fastify example](/examples/fastify) for a complete guide with drain, enrichers, tail sampling, and error handling.
::

### NestJS

evlog provides a NestJS module at `evlog/nestjs` with `EvlogModule.forRoot()`. It auto-creates a request-scoped logger accessible via `req.log` or `useLogger()` and emits a wide event when the response finishes.

```bash
npm install evlog @nestjs/common @nestjs/core @nestjs/platform-express
```

Register `EvlogModule` in your app module:
...
Initialize evlog in your bootstrap and create the NestJS app:
...
Use `useLogger()` to access the request-scoped logger from any controller or service:
...
The module supports the full evlog pipeline — drain adapters, enrichers, and tail sampling:
...
For async configuration (e.g. from `ConfigService`), use `forRootAsync()`:
...
Handle errors with a NestJS exception filter using `useLogger()` and `parseError()`:
...
::callout{icon="i-lucide-arrow-right" color="neutral"}
See the [NestJS example](/examples/nestjs) for a complete guide with drain, enrichers, tail sampling, and error handling.
::

### SvelteKit

SvelteKit integration via `handle` and `handleError` hooks in `hooks.server.ts`. Access the logger via `event.locals.log` in route handlers or `useLogger()` from services.
...
::callout{icon="i-lucide-arrow-right" color="neutral"}
See the [SvelteKit example](/examples/sveltekit) for a complete guide with drain, enrichers, tail sampling, and error handling.
::
