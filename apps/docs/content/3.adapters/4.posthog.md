...
[PostHog](https://posthog.com) is an open-source product analytics platform. The evlog PostHog adapter sends your wide events to [PostHog Logs](https://posthog.com/docs/logs) via the standard OTLP format, giving you a dedicated log viewer with filtering, search, and tail mode — all using your existing PostHog API key.
...
Once your events are flowing, use the **Logs** tab in PostHog to query them:
...
For advanced use cases, you can use the lower-level functions to send custom events:

```typescript [server/utils/posthog.ts]
import { sendToPostHog, sendBatchToPostHogEvents, toPostHogEvent } from 'evlog/posthog'
